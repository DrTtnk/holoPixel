...
for it in range(n_iters):
    ...
    loss = 1.0 * l_map + 0.02 * l_spot + 0.5 * l_margin + 0.01 * d.coeff.pow(2).sum()
    if not torch.isfinite(loss):
        print(it, "NON-FINITE LOSS, stopping", flush=True)
        break
    loss.backward()
    torch.nn.utils.clip_grad_norm_([d.pose, d.coeff], 0.3)
    opt.step()
    if it % 25 == 0 or it == n_iters - 1:
        ...
        if v > 0.97 and torch.isfinite(loss):
            ...
            np.savez("/tmp/phase2b_adam.npz", pose=best[1].numpy(), coeff=best[2].numpy(), index=d.index)
