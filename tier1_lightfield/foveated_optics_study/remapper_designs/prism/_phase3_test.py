...
opt = torch.optim.Adam([
    {"params": [d.pose], "lr": 0.0015},
    {"params": [d.coeff], "lr": 0.0012},
])
t0 = time.time()
n_iters = 700
best = None
for it in range(n_iters):
    w_spot = 0.05 * min(1.0, it / 300.0)  # slow ramp: spot fought the mapping objective last time
    opt.zero_grad()
    l_map, l_spot, l_margin, valid, r_ach, spot_rms = ds.loss_fn(d, tx, tz, target_r, weight, pupil_xy, margin_deg=2.0)
    loss = 1.0 * l_map + w_spot * l_spot + 0.5 * l_margin + 5e-3 * d.coeff.pow(2).sum()
    ...
    if it % 25 == 0 or it == n_iters - 1:
        v = float(valid)
        print(it, "w_spot", round(w_spot, 4), "loss", float(loss), "map", float(l_map),
             "spot", float(l_spot), "margin", float(l_margin), "valid", v,
             "spot_rms_um", float(spot_rms.mean()) * 1000, "t", time.time() - t0, flush=True)
        if v > 0.97 and torch.isfinite(loss):
            best = (float(l_map), float(l_spot), d.pose.detach().clone(), d.coeff.detach().clone())
            np.savez("/tmp/phase3_adam.npz", pose=best[2].numpy(), coeff=best[3].numpy(), index=d.index)

print("done, best map/spot loss:", (best[0], best[1]) if best else None, flush=True)
